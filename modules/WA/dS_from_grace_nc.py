...
def read_mascon_info_(MASCON_DATA_FOLDER):
    MASCON_INFO = os.path.join(MASCON_DATA_FOLDER, 'nc','tws_grace.nc')
    df_info = open_nc(MASCON_INFO,chunksize=None,layer=0)
    time, lat, lon = df_info.indexes.values()
    df_ = df_info[:,:,:].to_dataframe()
    df =df_info[0,:,:].to_dataframe()
    df.reset_index(inplace=True)
    
    mascon_coords = list(zip(df['longitude'],df['latitude']))
    mascon_dates = time
    
    return df_,mascon_coords,mascon_dates
...
#    MASCON_DATA_FOLDER = r"D:\Data\GRACE\GSFC.glb.200301_201607_v02.4-ICE6G"
    ...
    MASCON_SOLUTION = os.path.join(MASCON_DATA_FOLDER)
    
    ## Create buffer 
    create_buffer(BASIN_SHP, BUFFER_SHP, BUFFER_DIST)
    
    ## Read mascon info
    df_info,mascon_coords,mascon_dates=read_mascon_info_(MASCON_DATA_FOLDER)
    ...
    data_lines = []
    for i in index_mascons_of_interest[0]:
        lat = mascon_coords[i][1]
        lon = mascon_coords[i][0]
        
        data_lines.append(np.array(df_info.loc[(df_info.index.get_level_values('latitude') == lat) & (df_info.index.get_level_values('longitude') == lon)]['tws']).astype(float))
    ...
    for mascon_index in index_mascons_of_interest[0]:
        ID = mascon_index+1
        lon_corner = mascon_coords[mascon_index][0]
        lat_corner = mascon_coords[mascon_index][1]
        lon_span = 1
        lat_span = 1
        w.poly([
                [[lon_corner - .25, lat_corner + .25],
                  [lon_corner - .25, lat_corner - .25],
                  [lon_corner + .25, lat_corner - .25],
                  [lon_corner + .25 , lat_corner + .25],
                  [lon_corner - .25 , lat_corner + .25]]
    ...
    weighted_average = np.nansum(weighted_line, 0)
